...
export function Sidebar({ className, onClose, onNavClick }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "flex h-full w-64 shrink-0 flex-col border-r border-stone-200 bg-[#FBFBF9]",
        className
      )}
    >
      <div className="flex h-14 items-center justify-between gap-2 border-b border-stone-200 bg-white px-4">
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="rounded-md p-1.5 text-stone-600 hover:bg-stone-100 hover:text-stone-900 lg:hidden"
      ...
      <div className="border-t border-stone-200 bg-white px-3 py-2.5 space-y-2">
        {profile && (
          <div className="rounded-lg border border-stone-200 bg-white px-3 py-2 min-w-0">
            ...
            <p className="text-[10px] text-stone-600 mt-0.5">{roleLabel}</p>
          </div>
        )}
        <button
          type="button"
          onClick={() => void signOut()}
          className="flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-stone-700 hover:bg-stone-100 transition-colors"
        >
          <LogOut size={14} />
          Sign out
        </button>
