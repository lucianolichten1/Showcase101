...
import type { ManualTransactionInput } from "@/domains/financial/bank-accounts/types";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-xl border border-stone-200 shadow-xl">
        <div className="flex items-center justify-between border-b border-stone-200 px-5 py-4">
          <h2 className="text-sm font-bold text-stone-900">{MANUAL_TRANSACTION_FORM_COPY.title}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-green-800 mb-1">
              {MANUAL_TRANSACTION_FORM_COPY.type}
            </label>
            <select
              value={type}
              ...
              className="w-full py-2 px-3 text-xs border border-stone-200 rounded-lg"
            >
              <option value="income">{MANUAL_TRANSACTION_FORM_COPY.income}</option>
              <option value="expense">{MANUAL_TRANSACTION_FORM_COPY.expense}</option>
            </select>
          </div>
          <div>
            <label className="block text-[10px] font-bold uppercase text-green-800 mb-1">
