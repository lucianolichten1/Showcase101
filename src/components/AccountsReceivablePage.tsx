...
export function AccountsReceivablePage() {
  ...
  const totalOutstanding = activeReceivables.reduce(
  ...
  const avgDaysOverdue =
    overdueRows.length > 0
      ? Math.round(
          overdueRows.reduce((sum, r) => sum + r.overdueDays, 0) / overdueRows.length
        )
      : 0;

  // Aging buckets (balance due per bucket)
            ...
            <tbody className="text-[11px] text-stone-800">
              {receivables.map((row) => {
                ...
                return (
                  <tr
                    key={row.id}
                    className="h-10 border-b border-stone-50 last:border-0 hover:bg-stone-50 cursor-pointer transition-colors"
                  >
