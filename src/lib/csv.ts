...
/**
 * Simple CSV parser for MVP demo files.
 * Production should use a proper parser (e.g. Papa Parse) for quoted commas,
 * multiline cells, and locale-specific decimal separators.
 */
export function parseSimpleCsv(text: string): { headers: string[]; rows: ParsedRow[] } {
  const lines = text.split(/\r?\n/).filter((line) => line.trim() !== "");

  if (lines.length === 0) {
    throw new Error("The file is empty. Please upload a CSV with headers and data.");
  }

  const headers = lines[0].split(",").map((cell) => cell.trim());

  if (headers.length === 0 || headers.every((h) => h === "")) {
    throw new Error("Could not read column headers from the CSV.");
  }

  if (lines.length === 1) {
    throw new Error("The CSV only contains headers. Add at least one data row.");
  }
...
export function detectColumns(headers: string[]): DetectedMapping {
  const mapping = Object.fromEntries(
    DETECTED_FIELDS.map((field) => [field, null])
  ) as DetectedMapping;

  const normalizedHeaders = headers.map((header) => ({
    original: header,
    lower: header.trim().toLowerCase(),
  }));
